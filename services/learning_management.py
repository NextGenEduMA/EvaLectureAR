...
class LearningManagementSystem:
    ...
    def update_student_progress(self, student_id: int, assessment_result: Dict) -> bool:
        ...
        try:
            ...
            if 'overall_score' in assessment_result:
                progress_record.average_score = float(self._update_average_score(
    ...
    def _update_average_score(self, student_id: int, new_score: float) -> float:
        ...
        try:
            ...
            # Convert numpy result to native Python float
    ...
    def _check_level_advancement(self, student: Student):
        ...
        try:
            ...
            avg_recent_score = float(np.mean(recent_scores))
    ...
    def _analyze_performance_trend(self, assessments: List[Assessment]) -> Dict:
        ...
        # Calculate score statistics - convert numpy types to native Python types
        score_stats = {
            'current_score': float(scores[-1]),
            'best_score': float(max(scores)),
            'average_score': round(float(sum(scores)) / len(scores), 2),
            'improvement_from_first': round(float(scores[-1]) - float(scores[0]), 2) if len(scores) > 1 else 0
        }
