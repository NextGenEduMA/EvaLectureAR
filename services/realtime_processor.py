...
class RealTimeAudioProcessor:
    ...
    def start_session(self, session_id: str, student_id, text_id) -> Dict:
        """Start a new real-time recording session"""
        try:
            # Validate student and text
            from models.database import Student, Text

            # Handle 'demo_student' student_id case
            if student_id == 'demo_student':
                # Get any available student for demo purposes
                student = Student.query.first()
                if not student:
                    return {
                        'success': False,
                        'message': 'لا توجد بيانات طلاب متاحة للتجربة'
                    }
            else:
            ...
            # Handle 'demo' text_id case
            if text_id == 'demo':
                # Get any available text for demo purposes
                text = Text.query.first()
                if not text:
                    return {
                        'success': False,
                        'message': 'لا توجد نصوص متاحة للتجربة'
                    }
            else:
                # Convert text_id to integer if it's a string
                try:
                    text_id = int(text_id)
                except (ValueError, TypeError):
                    return {
                        'success': False,
                        'message': 'معرف النص غير صالح'
                    }
            ...
            self.session_metadata[session_id] = {
                ...
                'text_id': text.id,  # Use the actual text ID
